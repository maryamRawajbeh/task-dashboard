...
export default function DashboardPage() {
  ...
  async function handleSave(e: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true)
    if (editTask) {
      const res = await fetch("/api/tasks", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, id: editTask.id }) })
  ...
  async function handleStatusChange(task: Task, newStatus: TaskStatus) {
    const res = await fetch("/api/tasks", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...task, status: newStatus }) })
    const updated: Task = await res.json()
    setTasks((prev) => prev.map((t) => (t.id === updated.id ? updated : t)))
  }
          ...
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} style={s.modalForm}>
              ...
              <div style={s.fieldGroup}>
                ...
                <input required type="date" value={form.due} onChange={(e) => setForm({ ...form, due: e.target.value })} style={{ ...s.input, colorScheme: "dark" }}
                  onFocus={(e) => (e.target.style.borderColor = "#7c6af7")} onBlur={(e) => (e.target.style.borderColor = "rgba(255,255,255,0.1)")} />
              </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={s.filterBar}>
              ...
              {hasFilters && (
                <button onClick={() => { setSearch(""); setFilterStatus("all"); setFilterPriority("all"); setFilterDue("") }} style={s.clearBtn}>
                  Clear ✕
                </button>
              )}
            </div>
                    ...
                    <tbody>
                      {filtered.map((task, i) => (
                        <tr key={task.id} style={{ ...s.tr, background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)" }}>
                          <td style={{ ...s.td, fontWeight: 500, maxWidth: "180px" }}>{task.title}</td>
                          <td style={{ ...s.td, color: "rgba(255,255,255,0.45)", maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {task.description || "—"}
                          </td>
                          ...
                          <td style={s.td}>
                            <div style={{ display: "flex", gap: "8px" }}>
                              <button onClick={() => openEdit(task)} style={s.editBtn}
                                ...
                                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(124,106,247,0.08)" }}>
                                Edit
                              </button>
